...
def set_sim_params(result_dir,cells,NP_cells,SE_cells,hs,k_se,simtime):
    ...
    par_name = [fpm_def,duty_def,dt_def,Ca_def,Wa_def,An_ini_def,Pn_ini_def,An_min_def,Pn_min_def,flash_amp_def,flash_time_def,testpar_def]
    ...
    with open(f'{result_dir}\\Default parameters.txt', 'w') as f:
        ...
        f.write(f'0 : Flash per min per cell (fpm) = {fpm_def}\n')
        f.write(f'1 : Duty ratio (duty) = {duty_def}\n')
        f.write(f'2 : Timestep (dt) = {dt_def}\n')
        f.write(f'3 : Natural Area shrinkage - Rate constant (Ca) = {Ca_def}\n')
        f.write(f'4 : Natural Area shrinkage - Threshold constant (Wa) = {Wa_def}\n')
        f.write(f'5 : Natural Area - Initial value (An_ini) = {An_ini_def}\n')
        f.write(f'6 : Natural Perimeter - Initial value (Pn_ini) = {Pn_ini_def}\n')
        f.write(f'7 : Natural Area - Minimum value (An_min) = {An_min_def}\n')
        f.write(f'8 : Natural Perimeter - Minimum value (Pn_min) = {Pn_min_def}\n')
        f.write(f'9 : Ca2+ flash amplitude (flash_amp) = {flash_amp_def}\n')
        f.write(f'10 : Ca2+ flash duration (flash_time) = {flash_time_def}\n')
    ...
    params_list = [fpm_sweep,duty_sweep,dt_sweep,Ca_sweep,Wa_sweep,An_ini_sweep,Pn_ini_sweep,An_min_sweep,Pn_min_sweep,flash_amp_sweep,flash_time_sweep,testpar_sweep]
 
    return par_name, params_list


def initialize_cells(CellGraph,VertexGraph,Boundary,NP_Boundary,cells,k_se,fpm,duty,dt,Ca,Wa,An_ini,Pn_ini,An_min,Pn_min,flash_amp,flash_time,testpar):
    '''
    ...
    cell_prob = (fpm*60)/(T_60/max_ref_time) if (fpm*60)/(T_60/max_ref_time) <= 1 else 1
    ...
    for i in range(cells):
        ...
        if cell_prob != 1:
            CellGraph.nodes[i]['Ca_conc'] = 0 if CellGraph.nodes[i]['SE'] != 1 else 0
            CellGraph.nodes[i]['Ca_timer'] = np.random.randint(0,maxref_frms+1) # timesteps until we reevaluate the occurrence of a Ca2+ flash in the cell
        else:
            CellGraph.nodes[i]['Ca_conc'] = 1 if CellGraph.nodes[i]['SE'] != 1 else 0
            CellGraph.nodes[i]['Ca_timer'] = int(flash_time/dt)    
...
def activate_cells(CellGraph,VertexGraph,Boundary,NP_Boundary,cells,k,fpm,duty,dt,flash_amp,flash_time):
    '''
        ...
        if (CellGraph.nodes[i]['Ca_timer'] == 0):
            if CellGraph.nodes[i]['Ca_conc'] == 1:
                if maxref_frms-flash_frms == 0:
                    ...
                    if (CellGraph.nodes[i]['Ca_conc'] == 0):
                ...
                if (CellGraph.nodes[i]['Ca_conc'] == 1):
                    ...
                    CellGraph.nodes[i]['Ca_timer'] = maxref_frms
            CellGraph.nodes[i]['Ca_amp'] = CellGraph.nodes[i]['Ca_conc']*flash_amp
...
def compute_cell_shape(CellGraph,VertexGraph,i,dt):
    '''
...
def simulate_VM(result_dir,directory,CellGraph,VertexGraph,Boundary,NP_Boundary,TissueCentre_x,TissueCentre_y,cells,vertices,simtime,hs,k_se,dx,dy,default_parameters,DEFAULT=True,par_ctr=None,par_ind=None,par=None,s_no=None,q_tstep=None,q_full=None,q_errp=None):
    ...
    fpm,duty,dt,Ca,Wa,An_ini,Pn_ini,An_min,Pn_min,flash_amp,flash_time,testpar = par_name
    ...
    try:
        ...
        for k in range(0,time_slns):
            ...
            if k < int(4000/dt):
                flash_amp = flash_amp + dt*((0.7-0.6)/4000)
            else:
                flash_amp = flash_amp + dt*((0.9-0.7)/2000)
                fpm = fpm + dt*((0.35-0.25)/2000)
            ...
            if k == 0:
                initialize_cells(CellGraph,VertexGraph,Boundary,NP_Boundary,cells,k_se,fpm,duty,dt,Ca,Wa,An_ini,Pn_ini,An_min,Pn_min,flash_amp,flash_time,testpar)
                
            else:
                activate_cells(CellGraph,VertexGraph,Boundary,NP_Boundary,cells,k,fpm,duty,dt,flash_amp,flash_time)
            ...
            for i in range(cells):
                ...
                area_buf, peri_buf, na_buf, np_buf, ucell_buff = compute_cell_shape(CellGraph,VertexGraph,i,dt)
...
def generate_figs(read_dir,cells,hs,x_vc,y_vc,dt,An_min,Pn_min,time_slns,txtskip):
    ...
    if len(TotEng) <= 54000: 
        ...
        for k in f_snap:
            ...
            cb0.set_label('Ca2+ flash amplitude',fontsize=20)
...
def main():
    ...
    print('-------------------')
    print('---------------------------------------------------------')
    print('0 : Flash per min per cell (fpm)')
    print('1 : Duty ratio (duty)')
    print('2 : Timestep (dt)')
    print('3 : Natural Area shrinkage - Rate constant (Ca)')
    print('4 : Natural Area shrinkage - Threshold constant (Wa)')
    print('5 : Natural Area - Initial value (An_ini)')
    print('6 : Natural Perimeter - Initial value (Pn_ini)')
    print('7 : Natural Area - Minimum value (An_min)')
    print('8 : Natural Perimeter - Minimum value (Pn_min)')
    print('9 : Ca2+ flash amplitude (flash_amp)')
    print('10 : Ca2+ flash duration (flash_time)')
    print('---------------------------------------------------------')
    print('')
